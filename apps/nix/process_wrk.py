...
def wrk_csv_cols() -> str:
    return """lat_avg(ms),lat_stdev(ms),lat_max(ms),req_avg,req_stdev,req_max,tot_requests,tot_duration,read,err_connect,err_read,err_write,err_timeout, req_sec_tot, transfer_sec(bytes/s)"""
...
def wrk_data(wrk_output: Dict[str, Any]) -> str:
    return str(wrk_output.get('lat_avg')) + ',' + str(wrk_output.get('lat_stdev')) + ',' + str(
        ...
        wrk_output.get('transfer_sec'))


def get_bytes(size_str: str) -> Any:
...
def parse_wrk_output(wrk_output: str) -> Dict[str, Any]:
    ...
    for line in wrk_output.splitlines():
        ...
        if x is not None:
            retval['transfer_sec'] = get_bytes(x.group(1))
        x = re.search(
