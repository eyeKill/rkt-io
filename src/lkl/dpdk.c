...
static int __dpdk_net_rx(struct lkl_netdev *nd, struct iovec *iov, int cnt) {
    ...
    for (rm = nd_dpdk->rcv_mbuf[nd_dpdk->bufidx]; rm; rm = rm->next) {
        ...
        if (nd_dpdk->offload & BIT(LKL_VIRTIO_NET_F_MRG_RXBUF) && i == 0)
            offset = sizeof(struct lkl_virtio_net_hdr_v1);
        else if (nd_dpdk->offload & BIT(LKL_VIRTIO_NET_F_GUEST_TSO4) && i == 0)
            i++;
        else
            offset = sizeof(struct lkl_virtio_net_hdr_v1);
        ...
        while (r_size > 0) {
            if (i >= cnt) {
                fprintf(stderr, "dpdk-rx: buffer full. skip it. ");
                fprintf(stderr, "(cnt=%d, buf[%d]=%lu, size=%lu)\n", i, cnt,
                        iov[i].iov_len, r_size);
                goto end;
            }

            copylen = r_size < (iov[i].iov_len - offset)
                          ? r_size
    ...
    if (read > (mtu + sizeof(struct ether_hdr) +
        ...
        header->hdr_len = hdr_lens.l2_len + hdr_lens.l3_len + hdr_lens.l4_len;
    }
    ...
    fprintf(stderr, "dpdk-rx: len=%d mtu=%d type=%d, size=%d, hdrlen=%d\n",
            read, mtu, header->gso_type, header->gso_size, header->hdr_len);
#endif
...
static int sgxlkl_dpdk_tx(struct lkl_netdev *nd, struct iovec *iov, int cnt) {
     ...
     * safe (e.g.,
     * http://www.dpdk.io/ml/archives/dev/2014-February/001401.html),
...
static int sgxlkl_dpdk_rx(struct lkl_netdev *nd, struct lkl__iovec *iov,
                          int cnt) {
    struct lkl_netdev_dpdk *nd_dpdk;
    int read = 0;
    ...
    if (nd_dpdk->npkts == 0) {
        nd_dpdk->npkts = rte_eth_rx_burst(nd_dpdk->portid, 0, nd_dpdk->rcv_mbuf,
                                          MAX_PKT_BURST);
        if (nd_dpdk->npkts <= 0) {
            ...
            if (!nd_dpdk->busy_poll) {
                _lthread_yield_cb(lthread_self(), __scheduler_enqueue,
                                  lthread_self());
            }
            return -1;
        }
...
static int sgxlkl_dpdk_poll(struct lkl_netdev *nd) {
    ...
    if (nd_dpdk->close) return LKL_DEV_NET_POLL_HUP;
    /*
...
struct lkl_netdev *sgxlkl_register_netdev_dpdk(
    struct enclave_dpdk_config *dpdk_iface) {
